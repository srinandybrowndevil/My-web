...
export const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate }) => {
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex fixed top-0 left-0 w-64 h-screen bg-slate-950/95 border-r border-slate-800/80 backdrop-blur-2xl z-40 flex-col justify-between p-4 py-6 overflow-y-auto custom-scrollbar select-none">
        {/* TOP SECTION: MENU HEADER (NO LOGO) */}
        <div>
          <div className="px-3.5 py-3 border-b border-slate-800/80 mb-4 flex items-center justify-between">
            <span className="text-[11px] font-black tracking-[0.2em] uppercase bg-gradient-to-r from-blue-400 via-cyan-400 to-amber-400 bg-clip-text text-transparent flex items-center gap-1.5">
            ...
            {mainSidebarNav.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3.5 py-2.5 rounded-xl font-bold text-xs sm:text-sm flex items-center justify-between transition-all duration-200 group ${
                    isActive
                      ? 'bg-gradient-to-r from-blue-600/20 via-cyan-500/15 to-transparent text-cyan-400 border border-cyan-500/30 shadow-lg shadow-cyan-500/10'
        ...
        <div className="pt-4 border-t border-slate-800/80 space-y-2.5">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-800 transition-all text-xs font-semibold"
          >
            <div className="flex items-center gap-2">
              <Search className="w-3.5 h-3.5 text-cyan-400" />
              <span>Search</span>
            </div>
            <kbd className="text-[9px] font-black tracking-wider text-slate-400 bg-white/10 px-1.5 py-0.5 rounded">
              ⌘K
            </kbd>
          </button>
          ...
          <div className="flex items-center justify-between gap-2 pt-1">
            <div className="rounded-xl bg-slate-900/80 border border-slate-800 p-1">
              <ThemeToggle />
            </div>

            <button
              onClick={() => handleNavClick('contact')}
              className="flex-1 py-2 px-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-xs transition-all flex items-center justify-center gap-1 shadow-md shadow-amber-500/20"
            >
              <span>Get Quote</span>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950/90 backdrop-blur-xl border-b border-slate-800/80 z-40 px-4 flex items-center justify-between">
        {/* Mobile Sidebar Menu Trigger (NO LOGO) */}
        <button
          onClick={() => setIsMobileMenuOpen(true)}
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-200 hover:text-white font-extrabold text-xs"
        >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="rounded-xl bg-slate-900 border border-slate-800 p-0.5">
            <ThemeToggle />
          </div>

          <button
            onClick={() => handleNavClick('contact')}
            className="py-2 px-3.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-black text-xs"
          >
            Quote
          </button>
        </div>
